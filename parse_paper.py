...
embeddings = get_embeddings(doc.sentences)
for s, emb in zip(doc.sentences, embeddings):
    s.embedding = emb
doc.save()
...
for i, s in enumerate(doc.sentences):
    if len(s.references) > 0 and 'doi' in s.references[0]:
        print(i, s.references[0]['doi'], s.references[0]['label'])
